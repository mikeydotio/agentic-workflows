...
How the forge plugin turns a plan into a finished implementation while you walk away.
...
```
/forge ideate "your idea"
        |
    /ideate runs with forge-aware hints
...
### 1. Start with /forge ideate

Run `/forge ideate` and describe what you want to build. This invokes `/ideate` behind the scenes with forge-aware hints:
...
You can also use `/ideate` directly if you want the standard flow. Phase 4.5 still offers the forge handoff either way.

### 2. Plan approved -- automatic handoff

After you approve the plan, ideate's Phase 4.5 presents a choice:

- **Autonomous via forge** -- you walk away
- **Execute here** -- ideate's existing Phase 5, you stay and watch
- **Just the plan** -- you execute manually

If you came through `/forge ideate`, autonomous is the default. Pick it.

### 3. Work decomposes your plan

`/forge plan` reads PLAN.md and:
...
- Writes `.forge/plan-mapping.json` linking everything together
...
If something's blocked, read the evaluator feedback in the storyhook comments, make a decision, unblock the story, and forge picks it up on the next cycle.
...
| You need to intervene | `/forge stop` for graceful shutdown at any time. |
...
|---------|-------------|
| `/forge ideate` | Start from scratch -- invokes `/ideate` with forge-aware hints, then hands off to plan + run |
| `/forge init` | Validate storyhook setup, add required states |
| `/forge plan [file]` | Decompose PLAN.md into storyhook stories |
| `/forge run [--interval 15m]` | Start autonomous execution with auto-resume timer |
| `/forge resume` | Resume from pause (usually called by timer, not you) |
| `/forge status` | Dashboard of progress, blockers, retries |
...
All settings live in `.forge/state.json` and are set at `/forge run` time:
