---
name: sentry
description: Manage the sentry pre-tool-use safety hook. Control permission mode behavior, analysis settings, allowlists/blocklists, and test commands.
---

# Sentry — Safety Hook Manager

Sentry evaluates every tool call for safety before execution. It auto-allows known readonly commands, warns on destructive commands, and uses Claude AI as a fallback for uncertain ones. It is permission-mode-aware and can be selectively enabled/disabled per mode.
...
Parse the ARGUMENTS after `/sentry` to determine the action:

| Argument | Action |
|----------|--------|
| `status` or empty | Show current configuration |
| `enable <mode>` | Enable sentry in a permission mode |
| `disable <mode>` | Disable sentry in a permission mode |
...
Claude Code has four permission modes: `default` (normal), `plan`, `acceptEdits`, and `bypassPermissions`. Sentry can be enabled or disabled per mode.

**By default, sentry is enabled in all modes except `bypassPermissions`.**

### /sentry enable <mode>
Enable sentry in the given permission mode by removing it from the `disabled_modes` list.

Valid modes: `default`, `plan`, `acceptEdits`, `bypassPermissions`

```bash
# Read current disabled modes
current=$(grep '^disabled_modes:' ~/.config/sentry/config.yaml | sed 's/^disabled_modes: *//')
# Remove the mode from the list
updated=$(echo "$current" | tr ' ' '\n' | grep -v "^<mode>$" | tr '\n' ' ' | sed 's/ *$//')
# Write back
sed -i "s/^disabled_modes: .*/disabled_modes: ${updated}/" ~/.config/sentry/config.yaml
```

Report the change: "Sentry is now **enabled** in `<mode>` mode."

### /sentry disable <mode>
Disable sentry in the given permission mode by adding it to the `disabled_modes` list.
...
Report the change: "Sentry is now **disabled** in `<mode>` mode."

### /sentry status
Read and display `~/.config/sentry/config.yaml`. Format as a table showing each setting and its current value. Also:
- Check whether `ANTHROPIC_API_KEY` is set (report set/unset, never show the key)
- Show which permission modes sentry is active/disabled in:
...
### /sentry mode <standard|strict|permissive>
...
```bash
sed -i 's/^mode: .*/mode: <value>/' ~/.config/sentry/config.yaml
```

### /sentry ai <on|off>
Enable or disable AI fallback:
```bash
sed -i 's/^ai_enabled: .*/ai_enabled: <true|false>/' ~/.config/sentry/config.yaml
```

### /sentry model <model-name>
Change the AI model for fallback analysis:
```bash
sed -i 's/^ai_model: .*/ai_model: <value>/' ~/.config/sentry/config.yaml
```
Valid models: `claude-sonnet-4-6`, `claude-haiku-4-5-20251001`, `claude-opus-4-6`

## Allow/Block Commands

### /sentry allow <command-name>
Add a command to the custom always-allow list. Read current `custom_allow`, append the command, write back:
```bash
current=$(grep '^custom_allow:' ~/.config/sentry/config.yaml | sed 's/^custom_allow: *//')
sed -i "s/^custom_allow: .*/custom_allow: ${current} <command-name>/" ~/.config/sentry/config.yaml
```

### /sentry block <command-name>
Same as `allow` but for the `custom_pass` key. Commands on this list always pass to user for confirmation.

### /sentry unallow <command-name>
Remove a command from the `custom_allow` list.

### /sentry unblock <command-name>
...
```bash
echo '{"tool_name":"Bash","tool_input":{"command":"<the command>"},"permission_mode":"default"}' | bash ${CLAUDE_PLUGIN_ROOT}/hooks/sentry.sh
```
...
```bash
tail -20 "$(grep '^log_file:' ~/.config/sentry/config.yaml | sed 's/^log_file: *//')"
```
...
```bash
cp "${CLAUDE_PLUGIN_ROOT}/references/default-config.yaml" ~/.config/sentry/config.yaml
```
If `CLAUDE_PLUGIN_ROOT` is not available, write the defaults inline:
```bash
cat > ~/.config/sentry/config.yaml << 'EOF'
