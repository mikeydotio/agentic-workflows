...
This document maps forge operations to storyhook interfaces. Storyhook has two interfaces: **MCP tools** (preferred for batch operations and structured responses) and **CLI** (for simple state changes). No direct file manipulation of `.storyhook/` data files.
...
States must exist in `.storyhook/states.toml` first. Run `/forge init` before using custom states.
...
- **Invalid state**: State not in states.toml — run `/forge init`
